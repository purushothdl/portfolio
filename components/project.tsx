...
import ImageModal from "./ImageModal"; // Ensure this path is correct
...
export default function Project({
  ...
  // Auto slideshow effect for web type
  useEffect(() => {
    if (type !== 'web' || imageUrl.length <= 1) return;
  ...
  return (
    <>
        ...
        <div className="rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 border border-neutral-200/50 dark:border-neutral-800/50 bg-white dark:bg-neutral-900">
          ...
          <div className="relative w-full overflow-hidden" style={{ aspectRatio: '16/9' }}>
            <button
              ...
              className="absolute top-4 right-4 z-20 p-2 rounded-full bg-black/40 text-white hover:bg-black/60 hover:scale-110 transition-all duration-300 opacity-0 group-hover:opacity-100"
              aria-label="Enlarge image"
            >
              <FiMaximize size={20} />
            </button>
            {type === "mobile" ? (
              <div className="flex w-full h-full gap-2 p-2 overflow-x-auto snap-x snap-mandatory bg-neutral-50 dark:bg-neutral-800">
      ...
      {/* RENDER THE MODAL using AnimatePresence */}
      <AnimatePresence>
        {isModalOpen && (
          <ImageModal 
            images={imageUrl} 
